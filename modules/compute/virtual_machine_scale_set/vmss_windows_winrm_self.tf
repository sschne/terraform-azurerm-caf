...
resource "azurerm_key_vault_certificate" "self_signed_winrm" {
  ...
  certificate_policy {
    ...
    x509_certificate_properties {
      ...
      #   dns_names = flatten([
      #     for nic_key in var.settings.vmss_settings[local.os_type].network_interface_keys : format("%s.%s", try(local.network_interfaces[nic_key].internal_dns_name_label, azurecaf_name.windows[each.key].result), local.network_interfaces[nic_key].internal_domain_name_suffix)
      #   ])
      # }
    }
  }
