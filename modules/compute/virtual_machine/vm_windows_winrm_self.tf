...
resource "azurerm_key_vault_certificate" "self_signed_winrm" {
  ...
  name         = try(format("%s-winrm-cert", data.azurecaf_name.windows[each.key].result), format("%s-winrm-cert", azurecaf_name.legacy[each.key].result))
  ...
  certificate_policy {
    ...
    x509_certificate_properties {
      ...
      subject            = try(format("CN=%s", data.azurecaf_name.windows[each.key].result), format("CN=%s", azurecaf_name.legacy[each.key].result))
  ...
  lifecycle {
    ignore_changes = [
      name,
      certificate_policy[0].x509_certificate_properties[0].subject,
      certificate_policy[0].x509_certificate_properties[0].subject_alternative_names
    ]
  }
